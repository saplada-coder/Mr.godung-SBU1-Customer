...
type PendingExpense = { id: number; projectId: number; projectName: string; category: string; description: string; vendor: string | null; amount: number; expenseDate: string; receiptUrl: string | null; createdByName: string | null; createdAt: string }
  ...
  return (
    <>
      ...
      <section className="card mt">
        <div className="sec-h"><h2>ค่าใช้จ่าย — รออนุมัติ</h2><span className="cnt-chip" style={{ background: 'var(--accent)' }}>{data.expenses.length}</span></div>
        <div className="alist">
          {data.expenses.map((e) => {
            ...
            return (
              <div className="arow" key={e.id}>
                ...
                <div className="aw">
                  <div className="an">{e.description} <span style={{ color: cm.c, fontSize: 11.5 }}>· {cm.label}</span></div>
